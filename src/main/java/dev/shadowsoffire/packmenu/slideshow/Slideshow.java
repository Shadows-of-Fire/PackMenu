...
public class Slideshow {
    ...
    public static void render(ExtendedMenuScreen screen, GuiGraphics gfx, float partialTicks) {
        ...
        if (fading) {
            ...
            RenderSystem.setShaderTexture(0, PackMenu.slideshowTextures.get(nextIndex()));
    ...
    public static void tick(ClientTickEvent.Post e) {
        if (Minecraft.getInstance().screen instanceof ExtendedMenuScreen && !finished) {
            ...
            fading = ticks % (PackMenu.slideshowDuration + PackMenu.slideshowTransition) >= PackMenu.slideshowDuration;
            if (wasFading && !fading) {
                index = nextIndex();
                if (index == PackMenu.slideshowTextures.size() - 1) {
                    finished = true;
                }
            }
        }
    ...
    public static int getAlphaFade(float partial) {
        float counterProgress = (ticks + partial) % (PackMenu.slideshowDuration + PackMenu.slideshowTransition) - PackMenu.slideshowDuration;

        float durationTeiler = 1F / PackMenu.slideshowTransition;
